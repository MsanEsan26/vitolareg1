...
setInterval(() => {
  http.get('https://prisonersreg2.glitch.me');
}, 60000);
...
client.on("userUpdate", async (oldUser, newUser) => { //acebots 
  ...
  await client.channels.cache.get(kanal).send(new Discord.MessageEmbed().setColor("#00ff51").setDescription(` ${newUser} \`${tag}\` **Tagımızı alarak aramıza katıldı**`));
  await client.guilds.cache.get(sunucu).members.cache.get(newUser.id).roles.add(rol);  
  }//acebots
  if (!newUser.username.includes(tag) && client.guilds.cache.get(sunucu).members.cache.get(newUser.id).roles.cache.has(rol)) {
  await client.channels.cache.get(kanal).send(new Discord.MessageEmbed().setColor("#ff0000").setDescription(` ${newUser} \`${tag}\` **Tagımızdan ayrıldı!**`));
